...
int main(const int argc, const char *argv[]) {
	...
	if(forkstat == 0) { // child proc
		...
		printf("  [**Child proc: Using IPC_PRIVATE=%d] \n", IPC_PRIVATE);
		int shmid = shmget(IPC_PRIVATE, sh_resc_len, 0);
		if(shmid == -1) {
		...
		printf("  [**Parent proc: Using IPC_PRIVATE=%d] \n", IPC_PRIVATE);
		int shmid = shmget(IPC_PRIVATE, sh_resc_len, 0666 | IPC_CREAT | IPC_EXCL);
		if(shmid == -1) {
